...
import setHead from '../../web/seo'; // <-- default import (NO curly braces)
...
export default function HomeScreen({ navigation }) {
    ...
    useFocusEffect(
        React.useCallback(() => {
            if (Platform.OS === 'web') {
                document.title = 'DoCard – My Flashcards';
                setHead({
                    title: 'DoCard – Home',
                    description: 'Browse decks, see totals and your average score.',
                });
            }
        }, [])
    );
    ...
    const { totalDecks, totalCards, avgScore } = useMemo(() => {
        ...
        const avgScore = scored.length
            ? Math.round(scored.reduce((s, d) => s + (d.lastScore?.pct || 0), 0) / scored.length)
            : null;
        ...
        <View style={{ flex: 1, backgroundColor: bs.bg, padding: 16 }}>
            ...
            <Card style={{ borderRadius: 20, backgroundColor: '#343a40', borderColor: '#343a40' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={{ color: '#fff', fontWeight: '800', fontSize: 18 }}>My Flashcards</Text>
            ...
            <Card style={{ marginTop: 14 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <View style={{ alignItems: 'center', flex: 1 }}>
                        <Text style={{ fontSize: 18, fontWeight: '900', color: bs.text }}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => {
                    ...
                    const badgeVariant =
                        pct === null ? 'secondary' : pct >= 80 ? 'success' : pct >= 60 ? 'warning' : 'danger';
